...
class MusicFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        // Use coroutine to retrieve data from Ticketmaster API asynchronously
        lifecycleScope.launch {
            // Make GET request to retrieve data from Ticketmaster API
            val apiString = httpGet(rootUrl+"attractions"+".json?apikey="+apikey)
            // Parse JSON response using a custom parser function
            var parsedjson = jsonParser(apiString)

            if (parsedjson.isNotEmpty()){
                // If data is available, populate the list view with the retrieved data
                adapter = AttractionAdapter(requireActivity(), parsedjson)
                eventListview.adapter = adapter

                // Set on click listener for each item in the list view to navigate to the details fragment
                eventListview.setOnItemClickListener { parent, view, position, id ->
                    // Retrieve the attraction ID of the selected event and pass it to the details fragment
                    ...
                    val fragmentTransaction = this@MusicFragment.parentFragmentManager.beginTransaction()
                ...
                // If no data is available, display a message indicating that there are no music events planned at the moment
